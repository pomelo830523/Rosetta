...
$ConfigPath = Join-Path $KbDir "config\kb.config.yaml"
...
$McpJson = @{
    mcpServers = @{
        $ServerName = @{
            type    = "stdio"
            command = $VenvPython
            args    = @((Join-Path $KbDir "rosetta\kb_server.py"))
            env     = @{ PYTHONUTF8 = "1" }
        }
    }
...
$SelftestPath = Join-Path $KbDir "tests\selftest.py"
if (Test-Path $SelftestPath) {
    ...
    Write-Host "[4/4] 無 tests\selftest.py,略過(建議仿 BestHouse 版為你的 AP 寫一份)"
}

Write-Host ""
Write-Host "=== 完成 ==="
Write-Host "後續:"
Write-Host "  1. 編輯 config\kb.config.yaml 填入你的 AP 區塊"
Write-Host "  2. 各 AP repo 跑 codegraph 建圖後,執行:.venv\Scripts\python.exe scripts\index_all.py"
Write-Host "  3. 開發模式:重啟 Claude Code 後 /mcp 應可看到 $ServerName"
Write-Host "  4. 集中部署:`$env:KB_TRANSPORT='http'; .venv\Scripts\python.exe rosetta\kb_server.py"
