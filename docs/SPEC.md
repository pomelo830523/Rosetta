...
| 運算子 | enum:`eq` / `contains`,其他值拒絕 | 非自由字串,不進 SQL 內插 |
...
四種歧義訊號(S1~S3 為 server 端實作;S4 由 Claude 從回傳資料辨識):
...
### 4.9 跨 AP 聯合查詢(Phase 11,設計定案、待確認後實作)
...
### 4.10 變更歷史查詢(backlog,設計中——git 量體待評估)

**動機**:現值(query_db_config)與實作(search_code)有了,
「什麼時候改的、為什麼改」缺——答案在 migration 檔 + git 歷史。

**構想**:唯讀 tool `get_change_history(relative_path, app, limit)` 包
`git log --follow`(可選 `-L` 行範圍鎖定 symbol);輸出截斷。

**未排程原因**:git 歷史量體大——大 repo 的 `log -p` 輸出可能灌爆對話;
需先定策略再實作,開放問題:
1. 預設只回 commit 訊息清單(不含 patch),要 diff 時另指定單一 commit?
2. 以 `-L <start>,<end>:<file>` 鎖行範圍是否足以控制量體?(rename/重排會斷鏈)
3. limit 預設幾筆?migration 檔(如 V19)與一般原始碼是否分開處理?
...
- 歧義釐清(§4.8):模糊題 5 題應先以 KB 候選做選項式釐清、釐清後命中正確來源;
  既有清晰 10 題誤觸發反問 ≤ 1/10。
- 驗收自動化:`scripts/eval_e2e.py` 以 claude CLI headless 逐題實測 + 啟發式
  判分(清晰題含 expected 符號、模糊題像反問且命中候選;誤觸發另計),
  輸出 eval/E2E-RESULT.md;邊界案例人工複核。
