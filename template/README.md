...
> 架設:QUICKSTART.md;使用者:USER-GUIDE.md。

## 上手

```powershell
Copy-Item kb.config.yaml.example kb.config.yaml   # 填你的 AP 區塊(engine 留 auto)
...
.venv\Scripts\python.exe -X utf8 index_all.py --pull   # 日常排程只掛這支
```
...
| 檔案 | 用途 |
|---|---|
| `kb_server.py` | MCP server(7 個唯讀 tools;stdio / KB_TRANSPORT=http) |
| `kb_config.py` | 讀 `kb.config.yaml`(multi-AP,編輯即時生效) |
| `index_all.py` | 批次索引(--pull / --rebuild / --app;掛排程) |
| `semantic_index.py` | 單 AP 語意索引(--app [--rebuild]) |
| `extract_glossary.py` | 對照表骨架萃取(--app) |
| `setup.ps1` | venv + 依賴 + .mcp.json + selftest |
| `glossary/` | 每 AP 一份對照表(只需中文;只存名詞對應不存公式) |
...
- selftest 需按你的 AP 客製(參考 BestHouse 的 selftest.py)。
- `.ps1` 檔要 UTF-8 with BOM;venv 綁絕對路徑,搬移後重跑 setup.ps1。
